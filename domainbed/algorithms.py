...
class DREAME(ERM):
    ...
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DREAME, self).__init__(input_shape, num_classes, num_domains,
                                   hparams)
        self.num_models= hparams['DREAME_num_models']
        ...
        self.DREAME_networks= torch.nn.ModuleList([nn.Sequential(copy.deepcopy(self.featurizer),classifier_i) \
            for classifier_i in self.classifiers])
        self.DREAME_network_optimizers =[ torch.optim.Adam(
                network_i.parameters(),
                lr=self.hparams["lr_DREAME"],
                weight_decay=self.hparams['weight_decay']
            ) for network_i in self.DREAME_networks]
        del self.network
    ...
    def update(self, minibatches,val_minibatches, unlabeled=None):
        ...
        [optim_i.zero_grad() for optim_i in self.DREAME_network_optimizers]
        for network_i in self.DREAME_networks:
        ...
        inner_net_models= torch.nn.ModuleList([copy.deepcopy(network_i) for network_i in self.DREAME_networks])
        ...
        if self.compute_train_beta:
            ...
            for j in range(self.num_models):
                for i in range(len(val_minibatches)):
                    pred_i = self.DREAME_networks[j](val_minibatches[i][0])
                    loss_i = F.cross_entropy(pred_i,val_minibatches[i][1])
                    grad_i = torch.autograd.grad(loss_i,self.DREAME_networks[j].parameters(), allow_unused=True)
        ...
        for i in range(self.num_models):
                self.put_gradients(self.DREAME_networks[i], inner_net_models[i],1)


        # torch.nn.ModuleList([ self.put_gradients(self.DREAME_networks[i], inner_net_models[i],1)\
        #     for i in range(self.num_models)])
        ...
        if self.use_all_batches_for_all_models:
            for model_selected in range(self.num_models):
                for i in range(len(val_minibatches)):
                    ...
                    objective += self.hparams['DREAME_beta']*loss_i.item()

                    for p, g_j in zip(self.DREAME_networks[model_selected].parameters(), grad_i):
                        if g_j is not None:
                            p.grad.data.add_(
                                self.hparams['DREAME_beta'] * g_j.data )
            """
        ...
        [optim_i.step() for optim_i in self.DREAME_network_optimizers]
